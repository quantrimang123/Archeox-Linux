...
show_log_tail() {
  if [[ -f $ILV_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$ILV_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "ILV installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/ILV/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$ILV_INSTALL_LOG_FILE"
      else
        tail "$ILV_INSTALL_LOG_FILE"
      fi
      ;;
